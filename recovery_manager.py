...
class RecoveryManager:
    def __init__(self, context, roi):
        self.context = context
        self.roi = roi
        self.debug = context.debug
        self.timeout = 90.0
        
        self.base_path = os.path.join(os.path.dirname(__file__), "images", context.game_name)
    ...
    def run_recovery(self, find_and_click_with_retry_fn):
        ...
        if not find_and_click_with_retry_fn("Connection Lost OK", self._path("connection_lost.png"), self.context, roi=self.roi, attempts=3, abort_check=check_timeout):
